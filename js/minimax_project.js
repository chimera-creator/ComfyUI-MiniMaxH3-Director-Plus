...
const PROJECT_STYLES = `
  ...
  .mmxd-project-select, .mmxd-project-name { min-width:0; height:24px; box-sizing:border-box; background:#111; color:#ddd; border:1px solid #3a3a3a; border-radius:4px; padding:2px 5px; font-size:10px; font-family:inherit; outline:none; }
...
app.registerExtension({
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      uiWidget.computeSize = (width) => [Math.max(10, width || node.size?.[0] || 520), 58];
      ...
      const setProject = (value) => {
        ...
        for (const other of app.graph?._nodes || []) {
          ...
          if (link?.origin_id === node.id) other._mmxProjectRefresh?.(selectedProject);
        }
      };
      const renderOptions = () => {
        select.innerHTML = "";
      ...
      create.addEventListener("click", async () => {
        ...
        try {
          const document = await createProject(project);
